...
void Bureaucrat::signForm(Form &form)
{
	if(!(form.getSigned()))
	{
		if(getGrade() <= form.getGradeSign())
		{
			...
			std::cout << *this << RED << " can not sign " << WHITE << YELLOW << form.getName() << WHITE << " form, because his grade is not sufficient." << std::endl;
	}
	else if (form.getSigned())
	{
		if(getGrade() <= form.getGradeSign())
		{
			...
			std::cout << *this << RED << " can not sign " << WHITE << YELLOW << form.getName() << WHITE << " form, because his grade is not sufficient and it has been already" << GREEN << " signed.\n" << WHITE << BOLD << getName() << WHITE << RED << "! No more bureaucracy please!!!!" << WHITE << std::endl;
	}
}
