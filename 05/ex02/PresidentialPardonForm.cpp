...
PresidentialPardonForm::PresidentialPardonForm(PresidentialPardonForm const &src): Form(src.getName(), 25, 5), target(src.getTarget())
{
...
void PresidentialPardonForm::execute(Bureaucrat const & executor) const
{
	if(getSigned())
	{
		...
		{
			std::cout << YELLOW << target << WHITE << UNDERSCORE << " has been pardoned by Zafod Beeblebrox." << WHITE << std::endl;
			std::cout << getName() << " has been " << GREEN << "executed" << WHITE << " by " << executor << std::endl;
		}
		else
			std::cout << executor << RED << " can not execute " << WHITE << YELLOW << getName() << WHITE << " form, because his/her grade is not suffcient." << std::endl;
	}
	else
	{
		if(executor.getGrade() <= getGradeExecute())
		{
			...
			std::cout << executor << RED << " can not execute " << WHITE << YELLOW << getName() << WHITE << " form, because his/her grade is not suffcient and form has not been signed yet." << std::endl;
	}
}
