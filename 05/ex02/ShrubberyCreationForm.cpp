...
ShrubberyCreationForm::ShrubberyCreationForm(ShrubberyCreationForm const &src): Form(src.getName(), 145, 137), target(src.getTarget())
{
...
void ShrubberyCreationForm::execute(Bureaucrat const & executor) const
{
	if(getSigned())
	{
		...
		{
			std::string new_name = target + "_shrubbery";
			...
			ofs << "        |X|" << std::endl;
			ofs.close();
			std::cout << YELLOW << target << WHITE << " has been " << GREEN << "executed" << WHITE << " by " << executor << std::endl;
		}
		else
